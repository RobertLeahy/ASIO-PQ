...
static void fail (	::scope_exit<T> e) noexcept {
	...
	try {
		...
		asiopq::scope_exit<T> i(std::move(e));
...
SCENARIO("scope_exit objects invoke their stored functor when they go out of scope unless they are moved or released","[asiopq][scope_exit][scope]") {
	
	using asiopq::in_place;
	using asiopq::nullopt;
	using asiopq::optional;
	using asiopq::scope_exit;
...
SCENARIO("scope_exit objects obtained from the make_scope_exit factory invoke their stored functor when they go out of scope","[asiopq][scope_exit][scope]") {
	
	using asiopq::make_scope_exit;
	using asiopq::make_scope_fail;
	using asiopq::make_scope_success;
	using asiopq::nullopt;
	using asiopq::scope_exit;
	using asiopq::optional;
